...
# **Lesson 14 — React Server Components and Streaming**

---

### **Context / Motivation**

Up to now, all our lessons have focused on **React in the browser** — components rendered and hydrated on the client side.

But as apps grew larger, two challenges became unavoidable:

1. **Initial load performance** — users wait for JS bundles before they see meaningful content.
2. **Data-fetching duplication** — fetching on the server _and_ again on the client leads to extra round-trips and boilerplate.

React’s answer is **React Server Components (RSC)** —
...
### **The Evolution: From SSR → Streaming → Server Components**
...
Server Components go further than SSR — they’re not just rendered on the server once.
...
- Only hydrate what’s interactive.
...
| **Can fetch data?**                       | ✅ Directly (no hooks)                           | ⚠️ Must use APIs / hooks |
| **Can use hooks?**                        | ❌ No React hooks like `useState` or `useEffect` | ✅ Yes                   |
| **Bundle size impact**                    | 0 KB (not sent to browser)                       | Adds to JS bundle        |
| **Can import server-only code (DB, FS)?** | ✅ Yes                                           | ❌ No                    |
| **Can use event handlers?**               | ❌ No (not interactive)                          | ✅ Yes                   |
| **When re-run?**                          | On request or revalidation                       | On user interaction      |

---

### **Example 1 — A Simple Server Component**
...
There’s **no bundle cost**, and no extra data-fetching logic on the client.

---

### **Example 2 — Mixing Server and Client Components**

Some parts of your UI need interactivity — those are client components.
...
The browser **progressively paints** as data arrives — no blank page, no full reload.

---

### **Example 4 — Using RSC in Next.js 13+**

Next.js 13’s `app/` directory uses RSC by default.
...
This creates **time-sliced SSR** — a direct extension of concurrent rendering to the server.
...
These same primitives work identically across client and server — React simply extends their meaning into streamed output.
...
- No duplication between “API” and “fetch on client”.
...
- Server Components can’t use stateful or browser-only hooks (`useState`, `useEffect`).
- Data fetching must be synchronous or promise-based — no side effects during render.
...
Server Components close the final loop in React’s evolution —
...
> **React Server Components move data-fetching and heavy computation to the server, while keeping interactivity on the client — all unified through streaming and Suspense.**
...
Error Boundaries → recovery boundaries
```

React no longer thinks in “client vs. server apps.”
It now thinks in **layers of responsibility**, each chosen for the most efficient place to run.

---

Next, we’ll move to **Lesson 15 — Hooks Deep Dive: Custom Hooks, Composition, and Inversion of Control**,
where we’ll return to the client side and learn how hooks scale logic reuse and composability across large applications.
