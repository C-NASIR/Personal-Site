...
# **Lesson 15 — Hooks Deep Dive: Custom Hooks, Composition, and Inversion of Control**

---

### **Context / Motivation**

At this point, you already know React’s fundamental shift:

> _Components describe UI; hooks describe behavior._

Hooks are not just “a way to use state and effects in functions.”
They represent **a new architecture** — one where _logic_ can be shared, composed, and inverted without changing the component hierarchy.
...
Each solved some problems but introduced others — wrapper hell, prop-passing clutter, or tangled abstractions.
...
Let’s look at an example — tracking mouse position.
...
You’d either **copy-paste** the logic or build a **HOC**:
...
Hooks let you extract that logic cleanly — without changing your component structure.
...
✅ Each component gets its own independent logic instance.
✅ No wrapper components, no prop plumbing.
✅ Fully composable — you can combine hooks together easily.

---

### **Why Hooks Are Better Than Components Here**

Let’s summarize **why `useMousePosition()` is better than HOCs or Render Props**:

| Problem with Old Pattern                     | Hook Solution                               |
| -------------------------------------------- | ------------------------------------------- |
| Wrapper hell (HOC inside HOC)                | Hooks are flat — no extra layers            |
...
### **Example 2 — Online Status (Side-Effect + Cleanup)**
...
✅ Logic is reusable.
✅ No duplicated lifecycle methods.
✅ Cleanup is self-contained.

---

### **Example 3 — Combining Hooks: Composition Power**
...
They are **independent yet composable** — something class inheritance and HOCs could never achieve without complexity.

---

### **Example 4 — Custom Hooks Encapsulating Control Flow**

Let’s make a hook that handles async loading with error boundaries built-in.
...
✅ `useAsync` cleanly separates data fetching from rendering.
✅ Each use instance is isolated and declarative.
✅ Hooks now act as _mini controllers_ inside your component.

---

### **Example 5 — Inversion of Control (Hooks vs. Components)**

Suppose we want a polling feature that repeatedly refetches data every N seconds.

With components, you might try to make a “Polling” wrapper:
...
This looks clean, but it **forces control upward** — you have to structure your JSX around `<Poller>`.
...
This is **inversion of control** — hooks don’t own structure; components do.
...
- Don’t wrap everything in a custom hook “just because.”
- Keep logic near where it’s used until it becomes repetitive.

---

### **How Hooks Compose Better than Components**

Hooks compose like pure functions — by _calling each other_, not nesting.
...
Composition feels natural — no wrapper, no new hierarchy, no prop juggling.

---

### **Principle / Takeaway**

> **Hooks separate behavior from structure. Components describe “what to show.” Hooks describe “how it behaves.”**

In short:

| Concept       | Responsibility        | Typical Scope                          |
| ------------- | --------------------- | -------------------------------------- |
| **Component** | Declares UI layout    | “What does this screen look like?”     |
| **Hook**      | Encapsulates behavior | “How does this screen act or respond?” |
...
They complete React’s vision of _functions as the fundamental unit of reuse_.

---

Next, we’ll move to **Lesson 16 — Performance Patterns: Memoization, Batching, and Avoiding Wasted Renders**,
where we’ll tie together concurrency, transitions, and hooks to learn _how React optimizes updates_ and _how to help it do less work without breaking purity._
