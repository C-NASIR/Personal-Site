...
# **Lesson 13 — Error Boundaries: Containing Failures**
...
React needs a way to _recover gracefully_ — show a localized error message, keep the rest of the app running, and avoid a blank screen.

That’s what **Error Boundaries** are for.
...
function App() {
  return (
    <>
      <Header />
      <BuggyComponent /> {/* 💥 breaks entire tree */}
      <Footer />
    </>
  );
}
...
You typically use them to wrap parts of your UI that may fail — such as asynchronous code, third-party widgets, or experimental features.
...
- Data still loading → show `<Spinner />`
- Data fetch failed → show `<ErrorMessage />`

React internally treats Suspense and ErrorBoundary similarly — both are _boundaries in the tree_ where it can pause or recover.
...
Most teams don’t re-write this logic manually.
...
| Stage                        | Description                                           |
| ---------------------------- | ----------------------------------------------------- |
| **Render**                   | Child throws an error → React skips rendering subtree |
...
| **User triggers retry**      | Boundary resets → tries to render children again      |
...
so one day you’ll be able to write:
...
When `userId` changes, React remounts the subtree — clearing the error.
...
Error boundaries complete the reliability side of React’s concurrent model —
...
Both are React’s way of saying:

> “Let’s not crash or freeze — let’s degrade gracefully.”

In short:

```
Suspense → fallback while waiting (Promise)
ErrorBoundary → fallback while recovering (Error)
```
...
Next, we’ll move to **Lesson 14 — React Server Components and Streaming**,
where we’ll see how React extends Suspense and boundaries to the **server**, letting parts of the UI render asynchronously and stream progressively to the browser.

Would you like to continue to Lesson 14?
